...
INTENT_KEYWORDS = {
    "view_cart":      ["view cart", "show cart", "show my cart", "my cart", "what's in my cart", "cart items", "see cart",
                        "display cart", "check cart", "open cart",],
    ...
    "status": [
        ...
        "show my recent order", "last order", "recent order", "my last order","status of all orders",
        "status of orders","show all orders","show me all orders","all orders","list all orders",
    ],
...
def detect_intent(query: str, session: dict | None = None, memory: dict | None = None) -> tuple[str, float]:
    ...
    if re.search(r"\b(status of|show|list).{0,10}\b(all|every|my)?\s*orders?\b", q):
    ...
    if re.search(r"\bcart\b", q):
        boost("view_cart", 1.0, "cart_keyword_present")
    # ── Structural signals ───────────────────────────────────────────────────
...
async def chat_endpoint(req: ChatRequest, authorization: str = Header(None)):
    ...
    try:
        ...
        if intent == "repair":
            ...
            if re.search(r"\b(all|every|each|full|complete|all my|every single|history|past|previous|of all|of orders)\b",query.lower(),) or re.search(r"\b(list|show|status of).{0,10}(all\s+)?orders?\b", query.lower()):
